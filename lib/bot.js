...
// Entity — use proper Entity class
        // CRITICAL: runtime_entity_id is BigInt from bedrock-protocol
        // Number() conversion may overflow if value exceeds MAX_SAFE_INTEGER (2^53-1)
        let runtimeId
        if (typeof packet.runtime_entity_id === 'bigint') {
            if (packet.runtime_entity_id > BigInt(Number.MAX_SAFE_INTEGER)) {
                this.emit('error', new Error(`Entity ID ${packet.runtime_entity_id} exceeds MAX_SAFE_INTEGER - potential precision loss`))
            }
        ...
        // Server sends head position - subtract eye height to get feet position
        this.position = new Vec3(pos.x, pos.y - 1.62, pos.z)
        ...
        this.entity.entityUniqueId = packet.entity_id
        ...
        this.game.levelId = packet.level_id || ''
        // NOTE: levelType is semantically wrong here (level_id is not level type)
        ...
        // Modern Bedrock (1.16.210+) uses block_properties instead of block_palette
        if (packet.block_properties && this.registry) {
            this.registry.loadBlockPalette(packet.block_properties)
        } else if (packet.block_palette && this.registry) {
    ...
    loadPlugin(plugin) {
        ...
        try {
            ...
            this._loadedPlugins.delete(plugin)
            this._handleError(err)
        }
    }
     ...
     */
    waitForTicks(ticks, timeout = 30000) {
        return new Promise((resolve, reject) => {
            if (!this.physicsEnabled || !this._physicsInterval) {
                return reject(new Error('Physics not enabled or physics loop not running'))
            }
            let remaining = ticks
            const onTick = () => {
                remaining--
            ...
            setTimeout(() => {
                if (this.listenerCount('physicsTick') > 0) {
                    this.removeListener('physicsTick', onTick)
                    reject(new Error('waitForTicks timed out'))
                }
            }, timeout)
        })
    }
