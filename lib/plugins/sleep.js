...
function sleepPlugin(bot) {
    ...
    bot.client.on('set_entity_data', (packet) => {
        if (packet.runtime_entity_id !== bot._runtimeEntityId) return
    ...
    bot.sleep = function (bedBlock) {
        return new Promise((resolve, reject) => {
            ...
            // Look at the bed
            if (bot.lookAt) {
                bot.lookAt(bedBlock.position.offset
    ...
    bot.wake = function () {
        ...
        bot.client.queue('player_action', {
            ...
            position: bot.bedPosition ? {
                x: Math.floor(bot.bedPosition.x),
                y: Math.floor(bot.bedPosition.y),
                z: Math.floor(bot.bedPosition.z)
            } : { x: 0, y: 0, z: 0 },
