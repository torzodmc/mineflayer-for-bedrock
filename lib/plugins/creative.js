...
function creativePlugin(bot) {
    ...
    /**
     * Fly to a position (frame-rate independent using fixed tick rate).
     * Assumes 20 ticks/second (50ms per tick) for consistent physics.
     * @param {Vec3} target
     * @param {number} [speed=10] - blocks per second
     */
    bot.creative.flyTo = function (target, speed = 10) {
        ...
        const TICK_MS = 50
        const tickVelocity = speed * TICK_MS / 1000
        const factor = tickVelocity / dist
