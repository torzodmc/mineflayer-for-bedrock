...
function isWalkable(bot, x, y, z) {
    ...
    return block.isAir || !block.solid
}
    ...
    bot.pathfinder = {
        ...
        goto(goal, options = {}) {
            return new Promise((resolve, reject) => {
                ...
                const recalculateEvery = options.recalculateEvery || 20

                let path = astar(bot, bot.entity.position, goal, maxNodes)
                ...
                function moveToNext() {
                    ...
                    const shouldRecalculate = pathIndex - lastRecalculateIndex >= recalculateEvery ||
                        pathIndex >= path.length - 3 ||
                        (goal instanceof GoalFollow && goal.entity && goal.entity.position)

                    if (shouldRecalculate && !bot.pathfinder._stopRequested) {
                        lastRecalculateIndex = pathIndex
                        const newPath = astar(bot, bot.entity.position, goal, maxNodes)
