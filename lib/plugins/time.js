...
function timePlugin(bot) {
    ...
    bot.client.on('game_rules_changed', (packet) => {
        ...
        for (const rule of rules) {
            if (rule.name === 'doweathercycle') {
                bot.time.doWeatherCycle = rule.value === 'true'
            }
        }
    })
     ...
     */
    bot.waitForTime = function (targetTime, timeout = 60000) {
        return new Promise((resolve, reject) => {
            const check = () => {
                const current = bot.time.timeOfDay % 24000
                const diff = Math.min(
                    Math.abs(current - targetTime),
                    24000 - Math.abs(current - targetTime)
                )
                if (diff < 500) {
            ...
            const timer = setTimeout(() => {
                bot.removeListener('time', check)
                reject(new Error('waitForTime timeout'))
            }, timeout)
        })
    }
}
