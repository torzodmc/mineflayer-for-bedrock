...
function diggingPlugin(bot) {
    ...
    bot.client.on('level_event', (packet) => {
        // Event 2001 = block break particles (confirms break on server)
        if (packet.event === 2001 && bot.targetDigBlock) {
    ...
    bot.digTime = function (block) {
        ...
        if (hardness === 0) return 50     // instant break (1 tick)
    ...
    bot.dig = function (block, forceLook = true, digFace = 'auto') {
        return new Promise((resolve, reject) => {
            if (!block) return reject(new Error('No block specified'))
            if (!bot.canDigBlock(block)) return reject(new Error('Cannot dig this block'))
