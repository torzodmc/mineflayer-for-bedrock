...
function autoEatPlugin(bot) {
    bot.autoEat = {
        ...
        options: {
            ...
            minHealth: 1,             // Minimum health required to eat (Bedrock)
    ...
    // Monitor hunger (Bedrock sends 'health' event for food changes)
    bot.on('health', () => {
        if (!bot.autoEat.enabled) return
        if (bot.autoEat.eating) return
        if (bot.food > bot.autoEat.options.startAt) return
        if (bot.health !== undefined && bot.health < bot.autoEat.options.minHealth) return
    ...
    // Also check periodically in case health event didn't fire
    bot.autoEat._checkInterval = setInterval(() => {
        if (!bot.autoEat.enabled) return
        if (bot.autoEat.eating) return
        if (bot.food > bot.autoEat.options.startAt) return
        if (bot.health !== undefined && bot.health < bot.autoEat.options.minHealth) return
    ...
    bot.on('end', () => {
        if (bot.autoEat._checkInterval) {
            clearInterval(bot.autoEat._checkInterval)
            bot.autoEat._checkInterval = null
        }
    })
}
