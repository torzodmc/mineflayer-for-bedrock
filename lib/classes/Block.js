...
class Block {
    ...
    constructor(blockData, position, stateId) {
        ...
        // Properties
        this.hardness = blockData ? (blockData.hardness !== undefined ? blockData.hardness : -1) : -1
    ...
    getSignText() {
        ...
        const texts = []
        if (this.blockEntity.Message) {
            if (Array.isArray(this.blockEntity.Message)) {
                for (const msg of this.blockEntity.Message) {
                    if (msg) {
                        try {
                            const parsed = JSON.parse(msg)
                            if (parsed.translate) {
                                if (parsed.with && Array.isArray(parsed.with)) {
                                    let text = parsed.translate
                                    for (const param of parsed.with) {
                                        text = text.replace(/%s/, param.text || param.translate || param)
                                    }
                                    texts.push(text)
                                } else {
                                    texts.push(parsed.translate)
                                }
                            } else {
            ...
            } else if (typeof this.blockEntity.Message === 'string') {
                try {
                    const parsed = JSON.parse(this.blockEntity.Message)
                    texts.push(parsed.text || parsed.translate || '')
                } catch {
                    texts.push(this.blockEntity.Message)
                }
            }
        }
        for (let i = 1; i <= 4; i++) {
