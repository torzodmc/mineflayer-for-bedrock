...
class Registry {
    ...
    _init() {
        ...
        // Pre-seed block state cache with minecraft-data blocks by their ID.
        // Bedrock runtime IDs are FNV-1a hashes of block state NBT, so they
        // won't always match minecraft-data IDs. But for many common blocks
        // (especially default states), early IDs tend to align.
        // This gives us basic coverage; update_block packets fill the gaps.
        if (this._data.blocksArray) {
            for (const block of this._data.blocksArray) {
                this._blockStateCache.set(block.id, block)
            }
        }
    }
    ...
    loadBlockPalette(palette) {
        ...
        for (let runtimeId = 0; runtimeId < palette.length; runtimeId++) {
            ...
            // Strip minecraft: prefix to match minecraft-data keys
            const name = entry.name.replace('minecraft:', '')

            // Look up in minecraft-data for full block info (hardness, drops, etc.)
            const blockData = this._data.blocksByName[name]

            if (blockData) {
                this._blockStateCache.set(runtimeId, blockData)
            } else {
                // Block not in minecraft-data — create a minimal entry
                this._blockStateCache.set(runtimeId, {
                    id: runtimeId,
                    name: name,
